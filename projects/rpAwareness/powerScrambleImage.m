function [ scrambledImage ] = powerScrambleImage( inputImage )
...
%inAmp = abs(inFourier);
inPhase = angle(inFourier);

%This uses a trick to easily make correct scrambles of Fourier matrices 
...
outAmp=abs(fft2(randn(size(inputImage))));

%reconstruct the scrambled image from its complex valued matrix
scrambledImage=ifft2(outAmp.*exp(1i.*inPhase),'symmetric');
