function c = redbluecmapCS(m,varargin)
...
%   colormap with values between 3 and 11, or 256 (CS edited). Low values are dark
%   blue, values in the center of the map are white, and high values are
%   dark red. If M is empty, a default value of 256 will be used.
%
%   Example:
% 
...
switch (m)
    ...
    case 256
        ...
        for tmp = 1:10
            for rgb = 1:3
                y = orig(tmp):orig(tmp+1);
                c(y,rgb) = linspace(c(y(1),rgb),c(y(end),rgb),length(y));
            end
        end
