function [patchimgs,noiseimgs,rngs,cfg] = make_gabor_noise_CS(cfg)
...
[patchimgs,noiseimgs] = deal(zeros(patchsiz,patchsiz,numimage));
rngs = repmat(struct('Type',[],'Seed',[],'State',[]),1,numimage);
for ii = 1:numimage
    rngs(ii) = rng;
    ...
    % patchimg = (gaborimg+noiseimg); 
    patchimg = (gaborimg+noiseimg).*normpdf(r,0,patchenv)/normpdf(0,0,patchenv);
    patchimg = patchimg+patchlum;
    patchimg(r>patchsiz/2) = patchlum; % trim to circle (CS edit)
    patchimgs(:,:,ii) = patchimg;

    % make noise only patch
    noiseimg = noiseimg.*normpdf(r,0,patchenv)/normpdf(0,0,patchenv) + patchlum;
    noiseimg(r>patchsiz/2) = patchlum; % trim to circle (CS edit)
    noiseimgs(:,:,ii) = noiseimg;
end
end