...
function gabor = gabor(imSize,phase,thetaRad,sf,grey)
...
% % donut hole
% if nargin>5
%     X = 1:innerDiameter;                           % X is a vector from 1 to imageSize 
%     X0 = (X / imSize) - .5
%     [Xm Ym] = meshgrid(X0, X0);             % 2D matrices   
%     gaussInner = exp( -(((Xm.^2)+(Ym.^2)) ./ (2* s^2)) ); % formula for 2D gaussian
% end

% Now multply grating and gaussian to get a GABOR
gabor = grating.* gauss;                % use .* dot-product
% gabor = grating.* gaussInner;                % use .* dot-product
end
