...
%electrical bridge.
%
%Please cite as: Alschuler et al (2014). Clin Neurophysiol.
%
...
%
%Copyright © 2013-2014 by Daniel Alschuler
...
%
%History:
%  v1.0.01 07/14/2014 by DA
%    - First release (1.x.xx) version. Tested eBridge on over 600 sessions'
%      worth of EEG data with only one error.
%    - Bugfixes.
%  v0.1.02 04/04/2014 by DA
%    - Added Labels input option.
%    - Fixed bug in ED plot y-axis scale when EDcutoff is not found (-1).
%  v0.1.01 10/04/2013 by DA
%    -First open beta (0.1.xx) version.
...
if any(strcmpi('/?',varargin)) || any(strcmpi('/h',varargin)) || any(strcmpi('help',varargin))
  ...
  fprintf(['This function uses EEG data to compute an electrical distance (ED) matrix.\n',...
          ...
          '  the ED matrix, but can decrease the number of extracted epochs and thus\n',...
          '  decrease the accuracy of the function. \n',...
          '\n',...
          ...
          '                      x epochs.\n',...
          '         - .srate:    Variable containing the sample rate in Hz.\n',...
          '\n',...
          ...
          '               excluded. Does not require a flag. If Labels input is 0,\n',...
          '               use channel indices instead.\n',...
          ...
          '               Fraction of epochs that must be less than the automatically-\n',...
          '               calculated electrical distance cutoff for a channel to be\n',...
          '               flagged as bridged. Default is 0.5.\n',...
          ' ''Labels''      0  Assign channel indices as labels instead of looking for\n',...
          '                  labels stored in EEG.chanlocs.\n',...
          '               1  Use channel labels stored in EEG.chanlocs (default).\n',... 
          ...
          '               1  Bandpass filter (0.5 to 30 Hz) continuous data using the\n',...
          '                  EEGLAB pop_eegfilt function.\n',...
          '\n',...
          'OUTPUT:\n',...
...
assert(~isempty(EEG),'Input EEG data structure "%s" is empty.', inputname(EEGStructInd))
assert(all(isfield(EEG,{'data','srate'})), ['Problem with fields in input structure "%s". Must contain fields .data\n',...
                                       'and .srate.'],inputname(EEGStructInd))
assert(~(isempty(EEG.data) || isempty(EEG.srate)),'Problem with input structure %s. Fields data and srate cannot be empty.', inputname(EEGStructInd))
...
clear AllEEGStructs AllExChanCells
%Check flag inputs.
%flagCell contains flag names on first row and the corresponding defaults
%on the second row.
flagCell = {'BinSize','BCT','Labels','PlotMode','Verbose','EpLength','FiltMode';0.25,0.5,1,1,1,0,0};
...
assert((EBinput.Labels == 0) || (EBinput.Labels == 1),'Labels input must be "0" or "1".','')
...
%Check for labels.
if (EBinput.Labels == 1)
  assert(isfield(EEG,'chanlocs') && isfield(EEG.chanlocs,'labels'),['Electrode labels missing. Input structure "%s" must contain fields\n',...
                                                                  '.chanlocs and .chanlocs.labels, or Labels input must be 0.'],inputname(EEGStructInd))
  assert(~isempty(EEG.chanlocs) && all(~cellfun(@isempty,{EEG.chanlocs.labels})),['Problem with electrode labels. Fields .chanlocs and .chanlocs.labels in\n',...
                                                            'input structure "%s" cannot be empty, or Labels input must be 0.'], inputname(EEGStructInd))
end
clear EEGStructInd
%Check outputs.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if EBinput.Verbose == 2, fprintf('Please cite: Alschuler et al (2014). Clin Neurophysiol.\n'); end
...
if (EBinput.Labels == 1)
  ...
  AllChans = strtrim(cellstr(num2str([1:EB.Info.NumChansOrg]')))';
end
...
if deskind
  ...
  fprintf([begSpace, 'MATLAB desktop disabled; cannot display status bar. Please wait.\n']);
end
...
if deskind, fprintf([repmat(sprintf('\b'), 1, numDots+2+length(mBeg))]); end
...
if (EBinput.PlotMode == 2) || ((EBinput.PlotMode == 1) && (EB.Bridged.Count ~= 0))
  ...
  %Add annotation.
  EDplotText = cell(5,1);
  EDplotText(1,1) = cellstr(['eBridge.m ', char(169), 'D. Alschuler 2014']);
