function decoder = train_array2(cfg0, X, Y)
...
%                                                 Default = 'no'.
%                .window = [scalar]               How many samples to average before training (new to version 2)
%
...
for iDec = 1:numDec
    ...
    window = max(iDec-(cfg0.window-1)/2,1) : min(iDec+(cfg0.window-1)/2,numDec); % version 2
    curY = reshape( mean( Y(:, window, :) ,2) , [dimsSub, numN]); % version 2
%     curY = reshape(Y(:, iDec, :), [dimsSub, numN]); % original version
