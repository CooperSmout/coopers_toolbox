function Xhat = decode_array2(cfg0, decoder, Y)
...
%                                                 Default = 'no'.   
%                .window = [scalar]               How many samples to average before decoding (new to version 2)
%
...
Xhat_curDec = feval(cfg0.decodefun, cfg0.decodecfg, decoder{1}, squeeze( mean( Y(:,1:1+(cfg0.window-1)/2,:) ,2) ));
...
for iDec = 2:numDec
    ...
    window = max(iDec-(cfg0.window-1)/2,1) : min(iDec+(cfg0.window-1)/2,numDec);
    Xhat_curDec = feval(cfg0.decodefun, cfg0.decodecfg, decoder{iDec}, squeeze( mean( Y(:,window,:) ,2) ));
