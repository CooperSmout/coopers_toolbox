...
function [handle,Zi,grid,Xi,Yi] = topoplot_cs(Values,varargin)

% topoplot_cs() - plot a topographic map with presets
%
%   wrapper function, see TOPOPLOT for full documentation 
%
%   edited from EEGlab function TOPOPLOT.M by Cooper Smout (ORCID: 0000-0003-1144-3272)


%%%%%%%%%%%%%%%%%%%%%%%% Set defaults %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
icadefs                 % read defaults MAXTOPOPLOTCHANS and DEFAULT_ELOC and BACKCOLOR
BACKCOLOR = [1 1 1];  % white (previously [.93 .96 1])
whitebk = 'on';  % by default, make gridplot background color white
...
GRID_SCALE = 300;        % plot map on a 67X67 grid
...
if isstruct(Values) , fieldtrip == 1; end;
...
if ~exist('loc_file')
    try
        biosemi64 = evalin('base','biosemi64');
    catch
        load biosemi64.mat
        fprintf('loading chanlocs\n')
    end
    loc_file = biosemi64.chanlocs(1:64); 
end
if ~exist('MAPLIMITS')
    MAPLIMITS = 'absmax';
end
